...
public:
    ...
    friend BigInt operator - (const BigInt& a, const BigInt& b) {
        ...
        return -(a + b);*/
        if (c >= d) {// && a.sign && b.sign || !a.sign && !b.sign && b < a) {
            for (int i = 0; i <= c.CurSize; i++) {
                if (c.buf[i] < d.buf[i]) {
                    c.buf[i] += c.pow;
                    c.buf[i + 1] -= 1;
                }
                dif.buf[i] = c.buf[i] - d.buf[i];
            }
            //a = c;
            //b = d;
            for (int i = a.CurSize; i >= 0; i--)
                if (dif.buf[i]) {
                    dif.CurSize = i;
                    /*if (!a.sign)
                    return -dif;*/
    ...
    friend BigInt operator * (const BigInt& a, const BigInt& b) {
        BigInt mult;
        long long int * rem = new long long int[maxlen];
        for (int i = 0; i < a.maxlen; i++)
            rem[i] = 0;
        if (b.CurSize == 0 && b.buf[0] == 1)
            return a;
        if (a.CurSize == 0 && a.buf[0] == 1)
            return b;
        for (int i = 0; i <= b.CurSize; i++) {
            for (int j = 0; j <= a.CurSize; j++) {
                rem[j + 1] = (rem[j] + mult.buf[j + i] + a.buf[j] * b.buf[i]) / a.pow;
                mult.buf[j + i] = (rem[j] + mult.buf[j + i] + a.buf[j] * b.buf[i]) % a.pow;
                //rem[j + 1] = (rem[j] + mult.data[j + i] + a.data[j] * b.data[i]) / a.power;
            }
            mult.buf[a.CurSize + i + 1] = rem[a.CurSize + 1];
            if (i != b.CurSize)
                for (int i = 0; i <= a.CurSize + 1; i++)
                    rem[i] = 0;
        }
        if (rem[a.CurSize + b.CurSize + 1]) {
            mult.buf[a.CurSize + b.CurSize + 1] = rem[a.CurSize + b.CurSize + 1];
            mult.CurSize = a.CurSize + b.CurSize + 1;
        }
        for (int i = a.CurSize + b.CurSize + 1; i >= 0; i--) {
            if (mult.buf[i]) {
                mult.CurSize = i;
                break;
            }
    ...
    friend BigInt operator / (const BigInt& a, int b) {
        BigInt del;
        int* remain = new int[maxlen];
        for (int i = 0; i < a.maxlen; i++)
            remain[i] = 0;
        for (int i = a.CurSize; i > 0; i--) {
            del.buf[i] = (a.buf[i] + remain[i]) / b;
            remain[i - 1] = (a.buf[i] + remain[i] - del.buf[i] * b) * a.pow;
        }

        del.buf[0] = (remain[0] + a.buf[0]) / b;
        for (int i = a.CurSize; i > 0; i--)
            if (del.buf[i]) {
                del.CurSize = i;
                return del;
    ...
    friend BigInt operator / (BigInt& ia, BigInt& ib) {
        ...
        if (a - b == 0)
            return -1;
        BigInt c(a);
        bool sign = a.minusflag * b.minusflag;
        
        BigInt z(1);
        for (int i = 0; i < a.CurSize - b.CurSize + 1; i++)
            z = z * a.pow;
        while (c >= b) {
            while (c >= (b * z)) {
                c = c - (b * z);
...
BigInt operator / (const BigInt& left, const BigInt& right) {
    ...
    for (long long i = left.CurSize - 1; i >= 0; --i) {
        current._shift_right();//сдвиг всего массива вправо
...
BigInt operator / (const BigInt& left, const BigInt& right) {
    ...
    while (m >= ri) {
        ...
        if (m.CurSize > step) {
            ...
            int temp = m.CurSize;
            if (m.buf[m.CurSize - 1] - def.buf[def.CurSize - 1] == 0) {
                while (temp > 2 && m.buf[temp - 2] == 0) {
                    res = res * 10;
                    temp--;
                }
            }
    ...
    if (fl == 1 && prevlen == 1 && m.buf[prevlen] != 0) {
        res = res * 10;
    }
