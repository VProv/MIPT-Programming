...
// Получает максимальный поток с гарантированной прибавкой(случаями которые обсчитываются отдельно от графа). 
// Используется алгоритм Диница.
...
// Так как мы добавляем обратные ребра сразу после прямых, то XOR вернет обратное.
